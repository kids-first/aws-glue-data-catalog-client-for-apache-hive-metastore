...
sh """
 set +x
 curl 'https://api.github.com/repos/kids-first/aws-glue-data-catalog-client-for-apache-hive-metastore/statuses/$GIT_COMMIT?access_token=$GITHUB_TOKEN' -H 'Content-Type: application/json' -X POST -d '{\"state\": \"${status}\", \"description\": \"Jenkins\", \"target_url\": \"$BUILD_URL\"}'
 set -x
"""
}
